...
class ConversionService:
    def __init__(self, anthropic_api_key: str):
        logger.info(f"ConversionService init: key_prefix={anthropic_api_key[:20]} len={len(anthropic_api_key)}")
...
上記のPDF内容から、指定された列のデータを全行抽出してください。"""
        ...
        logger.info(f"Calling Anthropic API (text) with client key_prefix={str(self.client.api_key)[:20]}")
