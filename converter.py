...
10. フォームのラベル・項目名・空白フィールドも行として抽出する（値が空でもnullで含める）
...
class ConversionService:
    ...
    async def convert(
        ...
        if pdf_type == "text":
            result = await self._convert_text_pdf(pdf_bytes, column_definitions, auto_detect, anthropic_key)
            # テキストモードで失敗した場合はVisionで再試行
            if not result.get("success"):
                logger.info("Text mode failed, retrying with Vision mode")
                vision_result = await self._convert_scan_pdf(pdf_bytes, column_definitions, auto_detect, anthropic_key)
                if vision_result.get("success"):
                    return vision_result
                # 両方失敗した場合はテキストモードの結果を返す（より詳細なエラーの可能性）
            return result
        else:
            return await self._convert_scan_pdf(pdf_bytes, column_definitions, auto_detect, anthropic_key)

    # ─── テキスト抽出（構造化） ──────────────────────────────
    def _extract_text_structured(self, pdf_bytes: bytes) -> str:
        """テキストとテーブルを両方抽出して構造化テキストを返す"""
        pages_output = []
        with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
            for page_num, page in enumerate(pdf.pages, 1):
                parts = []

                # テーブル抽出（構造的なデータがある場合に優先）
                tables = page.extract_tables()
                if tables:
                    for table_idx, table in enumerate(tables):
                        parts.append(f"[テーブル {table_idx + 1}]")
                        for row in table:
                            if row:
                                # None を空文字に変換してタブ区切り
                ...
                if parts:
                    pages_output.append(f"=== ページ {page_num} ===\n" + "\n".join(parts))
...
テーブル形式のデータが含まれている場合は、その構造を参考にしてください。"""
    ...
    async def _convert_scan_pdf(
        ...
        if auto_detect:
            content.append({
                "type": "text",
                "text": "上記の画像（PDF）を解析し、含まれるデータを自動検出して全行抽出してください。表・明細行・フォームの記入値を含む全データを抽出してください。",
            })
            system = AUTO_DETECT_SYSTEM_PROMPT
        else:
            content.append({
...
上記の画像（PDF）から、指定された列のデータを全行抽出してください。""",
            })
            system = SYSTEM_PROMPT
